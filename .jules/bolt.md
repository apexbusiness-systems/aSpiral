...
## 2026-05-02 - Optimize Array Traversals\n**Learning:** Redundant array traversals, such as checking existence with `.some()` followed by extracting the item with `.find()`, introduce unnecessary O(N) operations in rendering paths.\n**Action:** Assign the result of `.find()` to a variable and reuse it for both checking existence and accessing the item's properties to cut traversal iterations in half.
