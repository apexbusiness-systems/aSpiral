...
## 2024-04-12 - Move timestamp conversion outside of iteration loops
**Learning:** Instantiating `Date` objects and utilizing heavy `date-fns` utilities like `isBefore`/`isAfter` inside an array `.filter` loop causes measurable overhead during iteration.
**Action:** Extract the timestamp conversions (`.getTime()`) outside of the filter array loop and use primitive numeric comparisons (`>` and `<`) to filter list data by date efficiently.
