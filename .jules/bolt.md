...
## 2025-05-18 - [O(N) Array Consolidations and GC Pressure Reduction in History Aggregation]
**Learning:** In the `getOverallStats` function within `src/lib/breakthrough/history.ts`, multiple sequential iterations (`for` loop, `.map()`, and `.reduce()`) over potentially large `entries` arrays created unnecessary intermediate memory allocations and redundant processing passes.
**Action:** Consolidate multiple metrics calculations (e.g., averages, sums, and unique Set extractions) into a single, unified pass over the data array. This minimizes GC pressure and maximizes O(N) efficiency for large user history sets.
