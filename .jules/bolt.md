...
## 2025-05-21 - Consolidate Sequential Iterations\n**Learning:** Sequential uses of `.filter()`, `.map()`, and `.reduce()` on the same array structure iterate the data multiple times (O(N) passes) and create intermediate array objects, contributing to GC overhead and reduced performance in critical code paths.\n**Action:** Consolidate sequential iterations into a single `for...of` loop to perform aggregation and mapping simultaneously, avoiding unnecessary allocations and duplicate passes.
